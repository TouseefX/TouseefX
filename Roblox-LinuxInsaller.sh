...
then
  sudo dpkg --add-architecture i386 && sudo mkdir -p755 /etc/apt/keyrings && sudo wget -O /etc/apt/keyrings/winehq-archive.key https://dl.winehq.org/wine-builds/winehq.key && $distro_update && sudo $distro_install --install-recommends winehq-staging;
fi
...
api="{\"FFlagDebugGraphicsPreferVulkan\": true"
 echo "installing Vulkan Driver Now"
 sleep 3
 $distro_install vulkan-tools vulkan-validationlayers libvulkan1 libvulkan-dev mesa_vulkan-drivers
 $distro_install mesa-vulkan-drivers
break
;;
"OpenGL")
